...
function simulate_variable_timestep!(NUM_NODE, gA, gB, gB2, gN, lifeA, lifeB, lifeB2)
    ...
    @inbounds for i = 1:2*NUM_NODE
        ...
        compute_node_perfstate!(NUM_NODE, gA, gB,gB2,  gN, lifeA, lifeB,lifeB2, idx)
...
function initialize!(NUM_NODE, gA, gB,gB2, gN)
    fill!(gA, 0)
    fill!(gB, 0)
    fill!(gB2, 0)
...
function compute_switchstate!(NUM_NODE, gA, gB,gB2, lifeA, lifeB, lifeB2)
    rand!(Exponential(1 / λA), lifeA)
    rand!(Exponential(1 / λB), lifeB)
    rand!(Exponential(1 / λB), lifeB2)
...
function compute_node_perfstate!(NUM_NODE, gA, gB, gB2, gN, lifeA, lifeB, lifeB2, idx) # ! todo
    ...
    #  ---+---+---+---+ 
    # gBi = 0
    if gB[idx] ==1 && lifeB[idx] == +Inf && gB2[idx] == 1 && gB2[idx] == +Inf
        gBi = 1
    elseif (gB[idx] == 2 && lifeB[idx]== +Inf) || (gB2[idx] == 2 && lifeB2[idx]== +Inf)
        gBi = 2
    else
        gBi = 0
    end
    if lifeA[idx] != +Inf
        gBi == 0 && (gN[idx] = 0; return nothing)
        gBi == 1 && (gN[idx] = 3; return nothing)
        gBi == 2 && (gN[idx] = 1; return nothing)
    elseif gA[idx] == 1
        gBi == 0 && (gN[idx] = 1; return nothing)
        gBi == 1 && (gN[idx] = 5; return nothing)
        gBi == 2 && (gN[idx] = 1; return nothing)
    elseif gA[idx] == 2
        gBi == 0 && (gN[idx] = 2; return nothing)
        gBi == 1 && (gN[idx] = 3; return nothing)
        gBi == 2 && (gN[idx] = 4; return nothing)
    elseif gA[idx] == 3
        gBi == 0 && (gN[idx] = 4; return nothing)
        gBi == 1 && (gN[idx] = 4; return nothing)
        gBi == 2 && (gN[idx] = 4; return nothing)
    end
    nothing
end
