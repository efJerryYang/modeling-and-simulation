...
function julia_main()
    ...
    # plot(system_life,legend-:topright,seriestype=:stephist,normalize=:pdf,title="Probability Density Function")
    p = histogram(system_life, legend=:topright, bins=min(NUM_SYSTEM, 256), normalize=:none, title="Monto-Carlo    Simulation", label="$(NUM_SYSTEM-bugval) valid samples")
    # p = histogram(system_life, legend=:topright, bins=NUM_SYSTEM, normalize=:pdf, title="Monto-Carlo Simulation", label="$(NUM_SYSTEM-bugval) valid samples")
    display(p)
